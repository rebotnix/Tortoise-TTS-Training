# Alpha Version for Tortoise TTS-Training
# Model was created from Whisper TTS
# Written by G. Hilgemann & L. Ickler
import argparse
import os
import warnings
...
def transcribe(
   ...
   # fp 16 is good for whisper API
    ...
    output_file = open(os.path.join(train_dir, "train_data.txt"), "w")
    ...
    with tqdm.tqdm(total=num_frames, unit='frames', disable=verbose) as pbar:
        while seek < num_frames:
            ...
            len_segments = len(all_segments)
            current_blocks = all_segments[new_last:]
            #split block by 2
            for el in [current_blocks[:math.floor(len(current_blocks)/2)], current_blocks[math.floor(len(current_blocks)/2):]]:
                start_time = el[0]["start"]
                end_time = el[-1]["end"]
                text = el[0]["text"]
                for i in el[1:]:
                    text = text + " " + i["text"]
                text = text.replace("  ", " ")
                output_snippet = train_dir+"/"+str(file_counter)+".mp3"
                cmd = "ffmpeg -y -i "+str(audio) + " -ss "+str(start_time)+" -to "+ str(end_time)+" "+ output_snippet
                p = subprocess.Popen(cmd , stdout=subprocess.PIPE, shell=True)
                (output, err) = p.communicate()  
                #This makes the wait possible
                p_status = p.wait()
                if os.path.exists(output_snippet):
                    output_file.write(text +"\t"+output_snippet+"\n")
                    file_counter = file_counter + 1

            new_last = len_segments-1
            # update progress bar
...
def cli():
    ...
    parser.add_argument("--train_dir", "-trr", type=str, default=".", help="directory to save the outputs")
    ...
    train_dir: str = args.pop("train_dir")
    ...
    for audio_path in args.pop("audio"):
        result = transcribe(model, train_dir, audio_path, temperature=temperature, **args)
